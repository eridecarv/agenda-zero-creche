    ...
    <Card padding="lg" className="overflow-hidden !p-0">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {complete && <span className="text-xs text-[#72AA78] font-medium">✓</span>}
        </div>
        {!disabled && (
          <span
      ...
      {open && !disabled && (
        <div className="px-5 pb-5 border-t border-[#F0E8E0] pt-5">{children}</div>
      )}
    </Card>
  )
}
...
export function DailyLogForm({ childId, schoolId, date, recordedBy, onSaved }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      const recordedMeals = (Object.entries(feeding) as [Meal, MealState][]).filter(
        ([, v]) => v.acceptance !== null
      )
    ...
    <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {[
              ...
              <Button
                ...
                variant="ghost"
                customColor={
                  hygiene[item.key as 'bath' | 'brushing' | 'bowelMovement'] ? '#EAF3DE' : '#F5EFE8'
                }
                customTextColor={
                  hygiene[item.key as 'bath' | 'brushing' | 'bowelMovement'] ? '#3A7A42' : '#8C7060'
                }
                onClick={() =>
                  setHygiene((prev) => ({
                    ...prev,
      ...
      >
        <textarea
          ...
          "
        />
        <p className="text-xs text-[#B0A090] mt-2">O recado será enviado assim que você salvar.</p>
      </Section>
        ...
        {checkOutRecorded ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-xs text-[#B0A090]">
              O dia desta criança está encerrado no app dos responsáveis. Você pode corrigir
              salvando novamente.
            </p>
            {/* Permite corrigir */}
                  ...
                  {guardians.map((g) => (
                    ...
                    >
                      <span className="flex items-center gap-3 w-full">
                        ...
                        {pickedUpBy === g.user_id && <span className="text-[#FF8C66]">✓</span>}
                      </span>
                    </Button>
                  ))}
                </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            {guardians.length > 0 ? (
              ...
              <p className="text-xs text-[#C4A882]">Nenhum responsável vinculado a esta criança.</p>
            )}

            <p className="text-xs text-[#B0A090]">
              Ao salvar com a saída preenchida, o dia será encerrado no app dos responsáveis.
            </p>
          </div>
        )}
      </Section>
      ...
      {savedAt && <p className="text-center text-xs text-[#72AA78]">✓ Salvo às {savedAt}</p>}
    </div>
  )
}
