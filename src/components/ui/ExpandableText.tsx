...
export function ExpandableText({ text, detail, author }: ExpandableTextProps) {
    ...
    <span>
      ...
      {open && (
        <span className="block mt-2 rounded-[10px] bg-[#FAF7F2] px-3 py-2 border-l-2 border-[#E8E0D8]">
          <span className="block text-sm text-[#8C7060] leading-relaxed">"{detail}"</span>
          {author && <span className="block mt-1 text-xs text-[#C4B5A8]">{author}</span>}
        </span>
      )}
    </span>
  )
