...
export async function createIncident(
  ...
  try {
    ...
    if (input.attachment) {
      ...
      if (uploadError) {
        ...
        return {
          ok: false,
          error:
            "Erro ao enviar a foto. A ocorrência não foi registrada. Tente novamente.",
        };
      }
    ...
    if (incidentError || !incident) {
      console.error(
        "[createIncident] erro ao inserir:",
        JSON.stringify(incidentError),
      );
      return {
        ok: false,
    ...
    if (attachmentUrl) {
      ...
      if (attachmentError) {
        console.error(
          "[createIncident] erro ao salvar anexo:",
          attachmentError,
        );
      }
    }
