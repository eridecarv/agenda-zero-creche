...
export default function IncidentsPage() {
  ...
  const [openIncident, setOpenIncident] = useState<IncidentWithDetails | null>(null)
    ...
    <div className="min-h-screen bg-[#FAF7F2] pb-24">
      ...
      <div className="bg-[#FFFDF9] px-5 pt-12 pb-5 shadow-[0_2px_8px_rgba(180,140,120,0.08)]">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="font-display text-xl font-bold text-[#3A2E24]">Ocorrências</h1>
        </div>
      ...
      <div className="px-5 pt-4 max-w-lg mx-auto flex flex-col gap-2">
        {loadingList ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[#B0A090]">Nenhuma ocorrência em {formatMonth(month)}.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex flex-col gap-1 min-w-0">
                    ...
                    <span className="text-xs text-[#8C7060]">{incident.children?.name}</span>
                  </div>
      ...
      {createOpen && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#FFFDF9] rounded-t-[28px] shadow-[0_-4px_24px_rgba(180,140,120,0.18)] px-5 pt-5 pb-10 max-w-lg mx-auto max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="font-display text-lg font-bold text-[#3A2E24] mb-4">Nova ocorrência</h2>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                {createErrors.child && (
                  <span className="text-xs text-[#E86C88]">{createErrors.child}</span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-sm font-medium text-[#3A2E24]">Descrição</label>
                <textarea
                ...
                {createErrors.description && (
                  <span className="text-xs text-[#E86C88]">{createErrors.description}</span>
                )}
              </div>

              {/* Foto opcional */}
              <div className="flex flex-col gap-2">
                <span className="text-sm font-medium text-[#3A2E24]">Foto (opcional)</span>
                <input
                  ref={fileInputRef}
                  type="file"
                ...
                {newFile ? (
                  <div className="flex items-center justify-between bg-[#FAF7F2] rounded-[12px] px-4 py-3 border border-[#E8E0D8]">
                    <span className="text-sm text-[#3A2E24] truncate">{newFile.name}</span>
                    <button
                      type="button"
                      onClick={() => {
                        setNewFile(null)
                ...
                {newFileError && <span className="text-xs text-[#E86C88]">{newFileError}</span>}
              </div>

              {createErrors.general && (
                <span className="text-xs text-[#E86C88] text-center">{createErrors.general}</span>
              )}
              ...
              <>
                ...
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className="text-xs font-medium px-2 py-0.5 rounded-full"
                    style={{
                      background: STATUS_STYLE[openIncident.status as IncidentStatus].bg,
                      color: STATUS_STYLE[openIncident.status as IncidentStatus].text,
                    }}
                  >
                    {STATUS_LABEL[openIncident.status as IncidentStatus]}
                  </span>
                  <span className="text-xs text-[#8C7060]">
                    {new Date(openIncident.created_at).toLocaleDateString('pt-BR', {
                      day: '2-digit',
                      month: 'long',
                      year: 'numeric',
                    })}
                ...
                <p className="text-xs text-[#8C7060] mb-2">{openIncident.children?.name}</p>
                ...
                {/* Foto */}
                {openIncident.incident_attachments?.map((att: IncidentAttachment) => (
                  <a
                    key={att.id}
                    href={att.url}
                    target="_blank"
                  ...
                  >
                    ...
                    <span className="text-xs text-[#8C7060] ml-auto shrink-0">Abrir ↗</span>
                  </a>
                ))}
                ...
                <div className="flex flex-col gap-1 mb-5">
                  ...
                  {openIncident.sent_by_user && openIncident.sent_at && (
                    <p className="text-xs text-[#B0A090]">
                      ...
                      {new Date(openIncident.sent_at).toLocaleDateString('pt-BR', {
                        day: '2-digit',
                        month: 'long',
                      })}
                    </p>
                ...
                {isAdminOrCoord && openIncident.status !== 'sent' && (
                  <div className="flex flex-col gap-2">
                    ...
                    <Button variant="ghost" onClick={() => openEdit(openIncident)}>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {editErrors.description && (
                      <span className="text-xs text-[#E86C88]">{editErrors.description}</span>
                    )}
                  </div>

                  {editErrors.general && (
                    <span className="text-xs text-[#E86C88] text-center">{editErrors.general}</span>
                  )}
